...
public class VeerAi implements Ai
{
   ...
   private void stayAway(Spaceship mySpaceship, Spaceship otherSpaceship, ArrayList<ThrustCommand> thrustCommands)
   {
    ...
    private void avoidBorder(Spaceship mySpaceship, ArrayList<ThrustCommand> thrustCommands, float borderRadius)
    {
        ...
        if(Math.abs(mySpaceship.getXPosition() - borderRadius) > Math.abs(mySpaceship.getXPosition() + xAcc - borderRadius))
            escapeDirX *= -1;

        if(Math.abs(mySpaceship.getYPosition() - borderRadius) > Math.abs(mySpaceship.getYPosition() + yAcc - borderRadius))
            escapeDirY *= -1;
    ...
    public ArrayList<ThrustCommand> thrustCommands(Spaceship mySpaceship, Spaceship otherSpaceship, ArrayList<Bullet> bulletsPositions, float borderRadius)
    {
        ...
        if(waitMove(bulletsPositions, mySpaceship) && mySpaceship.getEnergy() > 300)
        {
            avoidBullets(thrustCommands);
        }
        else if((Math.abs(mySpaceship.getXPosition()) > borderThresh || Math.abs(mySpaceship.getYPosition()) > borderThresh) && mySpaceship.getEnergy() > 100)
        {
	    ...
	    else if(mySpaceship.getEnergy() > 350 &&
...
*/
		    ...
		    bulletXVel += .00001;
                 ...
                 // System.out.println(up);
                  ...
                  // up here ////////////////////////////////////////////////////////////////////////////////////////////////
                      ...
                      float velocityDirection = (float) Math.atan2(bulletYVel, bulletXVel);
                      float dir = mySpaceship.getDirection() + (float)Math.PI/2;
                    ...
                    if(escapeDirX != 0 || escapeDirY != 0) {
                        if (Math.abs(bulletYVel / bulletXVel) >= 7) {
                            ...
                            //.out.println(bulletYVel / bulletXVel + "\t" + xAcc + "\t" + up + "\t" + yAcc);
    ...
    public UpgradeData createStructure()
    {
        ...
        upgradeData.setUpgrade(UpgradeData.BATTERY_SIZE, 2);
        upgradeData.setUpgrade(UpgradeData.ENERGY_GENERATOR, 9);
